...
int main(int argc, char *argv[])
{
    ...
    auto stack3 = std::make_shared<MOCoroutine::Stack>(2048);
    auto stack4 = std::make_shared<MOCoroutine::Stack>(2048);
    ...
    int coroSharedId = 10;
    MOCoroutine::Coroutine coroShared("coroShared", std::move(stack1)/* sharedStack */, [&coroSharedId, &scheduler] () {
        for(int i = 0; i < 5; i++) {
            printf("id: %d\r\n", coroSharedId++);
    ...
    MOCoroutine::Coroutine coroShared1("coroShared1", std::move(stack2)/* sharedStack */, [&coroSharedId1, &scheduler] () {
        for(int i = 0; i < 5; i++) {
            printf("id: %d\r\n", coroSharedId1++);
            scheduler.yield();
        }
    ...
    MOCoroutine::Coroutine coroShared2("coroShared1", std::move(stack3)/* stack3 */, [&coroSharedId2, &scheduler] () {
        for(int i = 0; i < 5; i++) {
            printf("id: %d\r\n", coroSharedId2++);
    ...
    MOCoroutine::Coroutine coroShared3("coroShared1", std::move(stack4)/* stack3 */, [&coroSharedId3, &scheduler] () {
        for(int i = 0; i < 5; i++) {
            printf("id: %d\r\n", coroSharedId3++);
